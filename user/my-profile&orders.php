    ...
    <div class="container py-5">
        <div class="row">
            <div class="card mt-3 mx-auto shadow p-3 mb-5 bg-white" style="width: 40%; border-radius: 5%">
                ...
                <a href="edit-my-profile.php" class="text-decoration-none">
                    <h3 class="text-center mt-2 mb-1"><i class="fas fa-edit"></i> Edit Details</h3>
                </a>
            </div>
        </div>
    </div>
    ...
    <div class="container py-3 px-0 ">
        <div class="card rounded mb-5 shadow">
            <div class="row mx-0" style="height: 50px;">
                <button class="btn btn-secondary w-50 " onclick="window.location.href='my-profile&products.php'">My Products</button>
                <button class="btn-light w-50 border-0">My Orders</button>
            </div>
                  ...
                  <p class="card-description">
                    Add class <code>.table</code>
                        ...
                        <div class="table-responsive">
                                    <?php
                                    $sql = "SELECT * FROM `tbl_products` WHERE `p_id` IN
                                            (SELECT `p_id` FROM `tbl_orders` WHERE `buyer_id` = '$_SESSION[userid]')";
                                    $result1 = mysqli_query($conn,$sql); 
                                    if(mysqli_num_rows($result1)==0)
                                    {
                                        echo "<h1 class='text-center text-secondary my-5'>You haven't ordered anything yet!</h1>";
                                    }
                                    else{
                                        echo '
                                    ...
                                    {
                                        $sql = "SELECT * FROM tbl_orders WHERE p_id = $tbl_products[p_id]";
                                        $result2 = mysqli_query($conn,$sql);
                                        $tbl_orders = mysqli_fetch_assoc($result2);
                                        
                                        $sql = "SELECT * FROM tbl_users WHERE u_id = $tbl_products[u_id]";
                                        $result3 = mysqli_query($conn,$sql);
                                        $tbl_users = mysqli_fetch_assoc($result3);

                                        $date = date("j F, Y", strtotime($tbl_orders["datetime"]));
                                        ?>
                                        <tr>
                                            ...
                                            <?php echo '<td><a href="seller-profile.php?uid=' . $tbl_users['u_id'] . '" class="text-decoration-none">@' . $tbl_users['username'] . ' </a></td>'; ?>
                                            <td><?php echo $date ?></td>

                                              
                                            <td>
                                                <?php
                                                    if($tbl_orders['status']=='ordered')
                                                    echo '<span class="text-warning">Ordered</span>'; 
                                                    else if($tbl_orders['status']=='delivered')
                                                    echo '<span class="text-success">Recived</span>';
                                                    ?>
                                            </td>
                                            <td>
                                                <form action="" method="post">
                                                    <?php
                                                        if($tbl_orders['status']=='ordered')
                                                        {
                                                            echo '<input name="o_id" type="hidden" value='.$tbl_orders['order_id'].'>';
                                                            echo '<input name="p_id" type="hidden" value='.$tbl_orders['p_id'].'>';
                                                            echo '
                                                            <!--<button class="btn btn-outline-success" name="received">Recieved</button>-->
                                    ...
                                    if(isset($_POST['cancel']))
                                    {
                                        ...
                                        $sql = "UPDATE tbl_products SET `status` = 'posted', `active` = 'yes' WHERE p_id = '$p_id'";
                                        mysqli_query($conn,$sql);
                                        echo '<meta http-equiv="refresh" content="0; url=my-profile&orders.php">';
                                    }
                                    if(isset($_POST['received']))
                                    {
                                        $order_id = $_POST['o_id'];
                                        $p_id = $_POST['p_id'];
                                        $sql = "UPDATE tbl_orders SET status = 'delivered' WHERE `order_id` = '$order_id'";
                                        mysqli_query($conn,$sql);
                                        $sql = "UPDATE tbl_products SET status = 'delivered' WHERE `p_id` = '$p_id'";
                                        mysqli_query($conn,$sql);
                                        echo '<meta http-equiv="refresh" content="0; url=my-profile&orders.php">';
                                    }
                                ?>
